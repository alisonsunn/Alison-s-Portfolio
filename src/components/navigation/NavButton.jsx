...
const NavButton = ({ x, y, label, link, icon, newTab }) => {
  ...
  return (
    <div className='absolute cursor-pointer z-50' style={{ transform: `translate(${x}, ${y})` }}>
      <Link href={link} target={newTab ? '_blank' : '_self'} className='text-foreground rounded-full flex items-center justify-center bg-background/20 border border-pink-500/30 border-solid backdrop-blur-[6px] shadow-glass-inset hover:shadow-glass-sm group:' aria-label={label}>
        <span className='relative w-14 h-14 p-4 animate-spin-slow-reverse hover:text-pink-400 group-hover:pause'>
          {getIcon(icon)}
